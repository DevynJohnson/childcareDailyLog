...
export default function UsersDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const registeredUsers = userSnap.docs.map(
          (doc) => doc.data() as { email: string; role: string }
        );

        // Fetch all caregivers from caregivers collection
        const caregiverSnap = await getDocs(collection(db, "caregivers"));
        const caregiversList = caregiverSnap.docs.map(
          (doc) =>
            doc.data() as {
              email: string;
              firstName?: string;
        ...
        const caregivers = caregiversList.map((u) => ({
          ...
          registered: registeredUsers.some(
            (regUser) =>
              regUser.email === u.email && regUser.role === "caregiver"
          ),
        }));
        ...
        const parents: UserRow[] = uniqueParents.map((parent) => ({
          ...
          registered: registeredUsers.some(
            (regUser) =>
              regUser.email === parent.email && regUser.role === "parent"
          ),
        }));
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {error && (
        <div className="text-red-600 mb-2 p-2 bg-red-50 rounded">{error}</div>
      )}
      {success && (
        <div className="text-green-600 mb-2 p-2 bg-green-50 rounded">
          {success}
        </div>
      )}
